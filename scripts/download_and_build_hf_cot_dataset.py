#!/usr/bin/env python3
"""
Hugging Face CoT Thinking Dataset Downloader & Builder for RLF
===============================================================
Downloads high-quality open reasoning & thinking datasets from Hugging Face
(GSM8K step-by-step math, Alpaca instructions, OpenOrca reasoning)
and formats them with explicit <think>...</think> blocks for multi-hour
RLF model training.
...
def download_file(url: str, dest_path: Path, token: str = None):
    ...
    headers = {"User-Agent": "RLF-Dataset-Fetcher/1.0"}
    if token:
...
def process_gsm8k(input_file: Path, instructions: list, corpus: list, max_samples: int):
    """Process GSM8K step-by-step math reasoning dataset into <think> blocks."""
    print(f"[+] Processing GSM8K reasoning dataset ({input_file.name})...")
    count = 0
    with open(input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if "####" in raw_answer:
                ...
                rationale = f"<think> {reasoning_steps} </think>"
            ...
            if question and response:
                task_id = f"gsm8k_{count+1}"
                instructions.append(f"{task_id}\tarithmetic\t{question}\t{rationale}\t{response}\t1.0")
                corpus.append(f"{question} {rationale} {response}")
                count += 1
    print(f"[+] Formatted {count} GSM8K CoT thinking records.")

def process_alpaca(input_file: Path, instructions: list, corpus: list, max_samples: int):
    """Process Alpaca instruction dataset into reasoning & conversation rows."""
    print(f"[+] Processing Alpaca instruction dataset ({input_file.name})...")
    count = 0
    with open(input_file, "r", encoding="utf-8") as f:
        ...
        for data in data_list:
            ...
            rationale = f"<think> Step 1: Analyze prompt '{prompt}'. Step 2: Formulate response. </think>"
            response = f"{rationale} {output_text}"
            task_id = f"alpaca_{count+1}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Hugging Face CoT Reasoning Dataset Downloader for RLF")
    parser.add_argument("--output-dir", type=str, default="demo_data/preview_conversation", help="Target dataset directory")
    parser.add_argument("--max-samples", type=int, default=10000, help="Maximum reasoning samples to include (default: 10000)")
    ...
    if token:
        ...
        print("[+] No HF_TOKEN provided. Downloading public HF datasets...")
    ...
    # Read existing dialogues.tsv or fallback to base
    dialogues_file = out_dir / "dialogues.tsv"
    ...
    if gsm8k_cache.exists():
        process_gsm8k(gsm8k_cache, instructions, corpus, args.max_samples // 2)

    if alpaca_cache.exists():
        process_alpaca(alpaca_cache, instructions, corpus, args.max_samples // 2)

    # Write out expanded manifests
    ...
    if corpus_file.exists():
        with open(corpus_file, "r", encoding="utf-8") as f:
            existing_corpus = [line.strip() for line in f if line.strip()]
    
    combined_corpus = list(dict.fromkeys(existing_corpus + corpus))
    with open(corpus_file, "w", encoding="utf-8") as f:
        f.write("\n".join(combined_corpus) + "\n")

    print(f"\n=======================================================")
    print(f"[+] Successfully built large CoT reasoning dataset in {out_dir}:")
    print(f"    instructions_rows={len(instructions)-1}")
    print(f"    corpus_lines={len(combined_corpus)}")
    print(f"=======================================================")

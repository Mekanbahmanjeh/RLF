...
LanguageResponse HierarchicalLanguageFabric::generate_response(
    ...
    // Direct Episode Attractor Routing: require high confidence similarity (>= 0.75) for exact retrieval;
    // otherwise fall through to auto-regressive predictive text generation!
    if (!matches.empty() && matches.front().similarity >= 0.75 && !matches.front().episode->response.empty()) {
